...
class Vet:
    ...
    def heal(self, pet):
        if isinstance(pet, Pet):
            print(f"Ветеринар {self.name} лечит {pet.typee} по имени {pet.name}. Теперь он здоров!")
        else:
            print("Это не питомец!")
...
#git add . #вместо . можно написать название файла которое хочешь добавить (. доюовляет все файлы)
